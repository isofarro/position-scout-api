...
export function getGameHeader(db: Database, gameId: number) {
  ...
  const playerStmt = db.prepare('SELECT name FROM idx.players WHERE id = ?');
  const eventStmt = db.prepare('SELECT date, name FROM idx.events WHERE id = ?');
  const siteStmt = db.prepare('SELECT name FROM idx.sites WHERE id = ?');
  const openingStmt = db.prepare('SELECT name FROM idx.openings WHERE id = ?');
